...
In production this is enforced, not trusted: `public/_headers` serves
`/templates/*` under `default-src 'none'`, so a remote fetch, script, webfont or
form submission is blocked by the browser. A template that phones home looks
broken on the live site but fine locally — which is exactly why the grep matters
before merge.
